...
export const useUserStore = create((set, get) => ({
  ...
  getUsers: async () => {
    ...
    try {
      const res = await fetch('http://13.203.76.7:5000/api/users', {
        headers: {
  ...
  deleteUser: async (userId) => {
    ...
    set({ loading: true, error: null });
    try {
      const res = await fetch(`http://13.203.76.7:5000/api/users/${userId}`, {
        method: 'DELETE',
  ...
  loginUser: async (email, password) => {
    try {
      const res = await fetch('http://13.203.76.7:5000/api/users/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
